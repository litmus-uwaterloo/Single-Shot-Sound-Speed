% Time of Flight computation and sampling for Single Transmit Global Speed-of-Sound Estimation Example
% ver. 1.0  (Mar 2025 -- by Pat De la Torre, Di Xiao & Alfred Yu)
%
% SimplePseudoBeamformer.m
% 
% Usage: 
% This function gathers the correct samples after properly computing the ToF based on the SoS, the SoS assumed by the
% transducer and pixel positions. 
% Associated with the paper "One Shot, One SoS: A Real-Time, Single-Shot Global Speed of Sound Estimator" in Ultrasonics.

function [Imaged_tensor]=SimplePseudoBeamformer(rf_data , pos_z , pos_x , pos_array_x , sos_medium , tx_angle, sos_transducer, Fnumber, fs ,RXdelay)
% size(pos_z)=[Nz,1];
% size(pos_x)=[1,Nx];
...
if tx_angle>0
    tx_angle=asin(sind(tx_angle)*sos_medium/sos_transducer);
